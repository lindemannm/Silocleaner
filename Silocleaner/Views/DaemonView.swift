...
//            Spacer()
    ...
    private func getRuntimeStatus() -> [String: (pid: String?, status: String, isLoaded: Bool)] {
        ...
        let userResult = runDirectTool(executable: "/bin/launchctl", arguments: ["list"])
        if userResult.0 {
            parseStatusOutput(userResult.1, into: &statusMap, context: "user")
        }
        
        // Get system context services (requires sudo for full access)
        let systemResult = runDirectTool(executable: "/usr/bin/sudo", arguments: ["/bin/launchctl", "list"])
        if systemResult.0 {
...
struct LaunchItemRowView: View {
    ...
    private func executeLaunchctlCommand(_ action: String) {
        ...
        Task {
            ...
            // Keep service labels and plist paths as process arguments. They
            // originate outside the app and must not be parsed as shell input.
            switch action {
            case "load":
                if !item.path.isEmpty && item.path != "Not found" {
                    arguments = ["load", item.path]
                } else {
                    arguments = ["enable", "\(domain)/\(item.label)"]
                }
            case "unload":
                if !item.path.isEmpty && item.path != "Not found" {
                    arguments = ["unload", item.path]
                } else {
                    arguments = ["disable", "\(domain)/\(item.label)"]
                }
            case "kickstart":
                arguments = ["kickstart", "-k", "\(domain)/\(item.label)"]
            case "remove":
                arguments = ["remove", item.label]
            default:
                await MainActor.run {
            ...
            if needsSudo {
                ...
                let result = try! await runPrivilegedTool(
                    "/bin/launchctl", arguments: arguments,
                ...
                // User agents do not require an authorization prompt.
                let result = await Task.detached {
                    return runDirectTool(executable: "/bin/launchctl", arguments: arguments)
                }.value
                success = result.0
                output = result.1
                if !success {
            ...
            await MainActor.run {
                ...
                if success {
                    printOS("Successfully executed launchctl \(action)")
...
private func runDirectTool(executable: String, arguments: [String]) -> (Bool, String) {
    let task = Process()

...
struct PackageView: View {
        ...
        return await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                if #available(macOS 10.5, *) {
                    ...
                    let files = PKGManager.getPackageFiles(packageID: packageId, installLocation: package.installLocation)
    ...
    private func loadPackagesFromPKGAPI() async -> [PackageInfo] {
        return await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                if #available(macOS 10.5, *) {
                    let packages = PKGManager.getAllPackages()
...
private func runFastShellCommand(command: String) -> (Bool, String) {
    let tempFile = "/tmp/silocleaner_getBOMlist_\(UUID().uuidString)"

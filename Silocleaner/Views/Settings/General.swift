        ...
        VStack(spacing: 20) {
                ...
                content: {
                    VStack {
                            ...
                            VStack(alignment: .leading, spacing: 0) {
                                HStack(spacing: 0) {
                                    Text("Homebrew cleanup after uninstall")
                                        .font(.callout)
                                        .foregroundStyle(ThemeColors.shared(for: colorScheme).primaryText.opacity(1))
                                    InfoButton(text: String(localized: "When Homebrew cleanup is enabled, Silocleaner will check if the app you are removing was installed via Homebrew and remove the cache to keep everything synced up."))
                                }
                        ...
                        HStack(spacing: 0) {
                            ...
                            InfoButton(text: String(localized: "When this mode is enabled, clicking the Uninstall button to remove an app will also close Silocleaner right after.\nThis only affects Silocleaner when it is opened via external means, like Sentinel Trash Monitor, Finder extension or a Deep Link.\nThis allows for single use of the app for a quick uninstall. When Silocleaner is opened normally, this setting is ignored and will work as usual."))
                        ...
                        HStack(spacing: 0) {
                            ...
                            InfoButton(text: String(localized: "When enabled, Silocleaner will automatically scan for app updates in the background during launch. This way, updates are ready when you open the Updater view."))
                            ...
                            VStack(alignment: .leading, spacing: 0) {
                                HStack(spacing: 0) {
                                    Text("Password cache timeout")
                                        .font(.callout)
                                        .foregroundStyle(ThemeColors.shared(for: colorScheme).primaryText)
                                    InfoButton(text: String(localized: "When running privileged Homebrew operations, Silocleaner caches your password in the macOS Keychain for this duration to avoid repeated password prompts. Homebrew commands cannot be executed with the privileged helper tool Silocleaner offers."))
                                }
                            }
            ...
            PearGroupBox(
                header: {
                    ...
                    The search sensitivity level controls how strict or lenient Silocleaner is when finding related files for an app:
                ...
                content: {
                    HStack(spacing: 0) {
                        ...
                        Text("Detect when apps are moved to Trash")
                            .font(.callout)
                            .foregroundStyle(ThemeColors.shared(for: colorScheme).primaryText)
                        InfoButton(text: String(localized: "When applications are moved to Trash, Silocleaner will launch and find related files and folders for deletion."))
                        Spacer()
                        ...
                        HStack(spacing: 0) {
                            ...
                            VStack {
                                ...
                                HStack(spacing: 0) {
                                    ...
                                    InfoButton(text: String(localized: "Enabling this extension lets you right-click apps in the standard Applications folders to quickly uninstall them with Silocleaner."))
                                    Spacer()
                                }

...
func manageSymlink(install: Bool, symlinkName: String = "pear") {
    ...
    if install {
        // Create the directory and link with fixed executables and separately
        // quoted arguments. `appPath` can contain arbitrary user-controlled
        // characters, so it must never be interpolated into shell source.
        if !binPathExists {
            Task {
                let mkdirResult = try? await runPrivilegedTool(
                    "/bin/mkdir", arguments: ["-p", "/usr/local/bin"]
                )
                guard mkdirResult?.success == true else {
                    printOS("Symlink directory creation failed")
                    return
                }

                let result = try? await runPrivilegedTool(
                    "/bin/ln", arguments: ["-s", appPath, symlinkPath]
                )
                if result?.success != true {
                    printOS("Symlink creation failed: \(result?.output ?? "unknown error")")
                }
                updateOnMain { isCLISymlinked = checkCLISymlink() }
            }
            return
        }
    } else {
...
func isRestricted(atPath path: URL) -> Bool {
    SilocleanerPathPolicy.isRestrictedApplication(path, protectedBundleURL: Bundle.main.bundleURL)
}
...
func sendStartNotificationFW() {
    DistributedNotificationCenter.default().postNotificationName(Notification.Name("Silocleaner.StartFileWatcher"), object: nil, userInfo: nil, deliverImmediately: true)
}

func sendStopNotificationFW() {
    DistributedNotificationCenter.default().postNotificationName(Notification.Name("Silocleaner.StopFileWatcher"), object: nil, userInfo: nil, deliverImmediately: true)
}
...
/// Generate system and Silocleaner environment information string for debugging
func getSystemDebugString() -> String {
    ...
    ====================================
    System & Silocleaner Debug Info
    ====================================
    Silocleaner Version: \(pcVersion) (Build \(pcBuild))

...
class HomebrewController: ObservableObject {
    ...
    func getOutdatedPackagesHybrid(formulae: [InstalledPackage], casks: [InstalledPackage]) async -> [HomebrewOutdatedPackage] {
        ...
        if !apiFailedPackages.isEmpty {
            ...
            // Filter out Silocleaner (has dedicated UI banner in Updater view)
            let allOutdated = coreOutdated + tapOutdated
            return allOutdated.filter { $0.name != "silocleaner" }
        }
        ...
        // Filter out Silocleaner (has dedicated UI banner in Updater view)
        return coreOutdated.filter { $0.name != "silocleaner" }
    }
    ...
    func calculateCaskSize(name: String) async -> (Int64, String) {
        // Special case for Silocleaner (running app, not in sortedApps)
        if name == "silocleaner" {
            let silocleanerPath = URL(fileURLWithPath: "/Applications/Silocleaner.app")
            guard FileManager.default.fileExists(atPath: silocleanerPath.path) else {
                return (0, "0 KB")
            }

            let totalBytes = totalSizeOnDisk(for: silocleanerPath)
            let bytesToFormat = totalBytes
            let formatted = await MainActor.run {
                ByteCountFormatter.string(fromByteCount: bytesToFormat, countStyle: .file)
            }
    ...
    private func calculatePKGOnlyCaskSize(caskName: String) async -> (Int64, String) {
        ...
        for identifier in pkgIdentifiers {
            if let receipt = receipts.first(where: { $0.packageId == identifier }) {
                totalSize += receipt.totalSizeFromBOM
            }
        }

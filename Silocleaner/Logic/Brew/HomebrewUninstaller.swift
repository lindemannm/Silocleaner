...
//  Silocleaner
//
//  Created by Silocleaner on 2025-10-03.
//
...
//        }
    ...
    private func uninstallCask(name: String, info: [String: Any], zap: Bool) async throws {
        ...
        if let artifacts = info["artifacts"] as? [[String: Any]] {
            ...
            for processName in processNamesToKill {
                _ = try? await runPrivilegedTool("/usr/bin/pkill", arguments: ["-9", "-f", processName])
                _ = try? await runPrivilegedTool("/usr/bin/killall", arguments: ["-9", processName])
            }
    ...
    private func handleEarlyScript(_ value: [String: Any]) async throws {
        // Homebrew metadata must not choose a root executable. Supporting these
        // arbitrary scripts requires a separately reviewed, typed operation.
        if value["executable"] != nil {
            UpdaterDebugLogger.shared.log(.homebrew, "Skipping unsupported cask early_script")
        }
    }
    ...
    private func handleLaunchctl(_ value: String) async throws {
        ...
        do {
            let bootout = try await runPrivilegedTool("/bin/launchctl", arguments: ["bootout", "system/\(value)"])
            if !bootout.success {
                _ = try? await runPrivilegedTool("/bin/launchctl", arguments: ["unload", systemPlistPath])
            }
            _ = try? await runPrivilegedTool("/usr/bin/pkill", arguments: ["-9", "-f", value])
        } catch {
            printOS("Failed to unload system service: \(error.localizedDescription)")
        }

        do {
            let bootout = try await runPrivilegedTool("/bin/launchctl", arguments: ["bootout", "gui/\(getuid())/\(value)"])
            if !bootout.success {
                _ = try? await runPrivilegedTool("/bin/launchctl", arguments: ["unload", userPlistPath])
            }
            _ = try? await runPrivilegedTool("/usr/bin/pkill", arguments: ["-9", "-f", value])
        } catch {
    ...
    private func handleQuit(_ value: String) async throws {
        do {
            _ = try await runPrivilegedTool("/usr/bin/killall", arguments: ["-15", value])
        } catch {
        ...
        do {
            _ = try await runPrivilegedTool("/usr/bin/pkill", arguments: ["-15", "-f", value])
        } catch {
    ...
    private func handleSignal(_ value: [Any]) async throws {
        ...
        guard signal.allSatisfy(\.isNumber) else { return }
        _ = try await runPrivilegedTool("/usr/bin/pkill", arguments: ["-\(signal)", process])
    }
    ...
    private func handleKext(_ value: String) async throws {
        _ = try await runPrivilegedTool("/sbin/kextunload", arguments: ["-b", value])
    }

    private func handleScript(_ value: [String: Any]) async throws {
        if value["executable"] != nil {
            UpdaterDebugLogger.shared.log(.homebrew, "Skipping unsupported cask script")
        }
    }

    private func handlePkgutil(_ value: String) async throws {
        // Get list of files from pkgutil
        let filesResult = try await runPrivilegedTool("/usr/sbin/pkgutil", arguments: ["--files", value])
        let files = filesResult.output.components(separatedBy: "\n").filter { !$0.isEmpty }
        ...
        // Forget the package
        _ = try await runPrivilegedTool("/usr/sbin/pkgutil", arguments: ["--forget", value])
    }

...
/// Releases temporary Bundle instances before a rescan. Foundation does not
/// expose a supported API for flushing its global bundle cache.
func flushBundleCaches(for apps: [AppInfo]) {
    for app in apps {
        autoreleasepool {
            _ = Bundle(url: app.path)
        }
    }
}
...
func getSortedApps(paths: [String], useStreaming: Bool = false) -> [AppInfo] {
    ...
    func collectAppPaths(at directoryPath: String) {
        let queue = DispatchQueue(label: "com.silocleaner.filetree", qos: .userInitiated, attributes: .concurrent)
        let group = DispatchGroup()
        let appsQueue = DispatchQueue(label: "com.silocleaner.apps.collection")
...
//    }
    ...
    if useStreaming {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let queue = DispatchQueue(label: "com.silocleaner.appinfo.mini", qos: .userInitiated, attributes: .concurrent)
            let group = DispatchGroup()

            var allMiniInfos: [AppInfoMini] = []
            let resultsQueue = DispatchQueue(label: "com.silocleaner.appinfo.mini.results")
        ...
        let chunks = createOptimalChunks(from: apps, minChunkSize: 10, maxChunkSize: 40)
        let queue = DispatchQueue(label: "com.silocleaner.appinfo.full", qos: .userInitiated, attributes: .concurrent)
        let group = DispatchGroup()

        var allFullInfos: [AppInfo] = []
        let resultsQueue = DispatchQueue(label: "com.silocleaner.appinfo.full.results")
...
func manageFinderPlugin(install: Bool) {
    ...
    task.arguments = ["-e", "\(install ? "use" : "ignore")", "-i", "com.lindemannm.Silocleaner.FinderOpen"]
...
private let caskLookupQueue = DispatchQueue(label: "com.silocleaner.cask.lookup", attributes: .concurrent)
                ...
                if let uninstalls = artifact["uninstall"] as? [[String: Any]] {
                    for uninstall in uninstalls {
                        ...
                        if let pkgutilID = uninstall["pkgutil"] as? String {
                            if let packageInfo = PKGManager.getAllPackages().first(where: { $0.packageId == pkgutilID }) {
                                let installedFiles = PKGManager.getPackageFiles(packageID: pkgutilID, installLocation: packageInfo.installLocation)
...
func exportDebugInfo(appState: AppState) {
    ...
    if panel.runModal() == .OK, let selectedFolder = panel.url {
        ...
        if appState.currentView == .files && !appState.appInfo.bundleIdentifier.isEmpty {
            ...
            filename = "SilocleanerDiagnostics-\(timestamp).txt"
        }
...
// --- Silocleaner Uninstall ---
func uninstallSilocleaner(appState: AppState, locations: Locations) {

    // Unload Sentinel Monitor if running
    launchctl(load: false)

    // Get app info for Silocleaner
    let appInfo = AppInfoFetcher.getAppInfo(atPath: Bundle.main.bundleURL)

    // Find application files for Silocleaner
    AppPathFinder(
        appInfo: appInfo!, locations: locations, appState: appState,
        completion: {
            // Kill Silocleaner and tell Finder to trash the files
...
func launchctl(load: Bool, completion: @escaping () -> Void = {}) {
    let service = SMAppService.agent(plistName: "com.lindemannm.SilocleanerSentinel.plist")

    if load {
        do {
            try service.register()
        } catch let error as NSError {
            printOS("Error registering SilocleanerSentinel: \(error)")
        }
    } else {
        do {
            try service.unregister()
        } catch let error as NSError {
            printOS("Error unregistering SilocleanerSentinel: \(error)")
        }
    }
...
func createTarArchive(appState: AppState) {
    ...
    do {
        ...
        if process.terminationStatus != 0 {
            ...
            throw NSError(
                domain: "com.lindemannm.Silocleaner.archiveExport",

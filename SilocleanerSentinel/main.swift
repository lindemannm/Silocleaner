...
func setupNotificationListener() {
    ...
    notificationCenter.addObserver(forName: Notification.Name("Silocleaner.StartFileWatcher"), object: nil, queue: nil) { notification in
        print("Received start notification")
        startGlobalFileWatcher()
    }
    notificationCenter.addObserver(forName: Notification.Name("Silocleaner.StopFileWatcher"), object: nil, queue: nil) { notification in
...
func checkApp(file: String) {
    ...
    if appExt == "app" {
        if let appBundle = Bundle(url: app) {
            if appBundle.bundleIdentifier == "com.lindemannm.Silocleaner" {
                return
            } else {
                if FileManager.default.isInTrash(app) {
                    NSWorkspace.shared.open(URL(string: "silocleaner://openApp?path=\(file)")!)
                }
            }
        } else {
